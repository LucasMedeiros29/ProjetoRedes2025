...
NS_LOG_COMPONENT_DEFINE("AodvStatic15NodesWithOverhead");
...
int main(int argc, char *argv[])
{
    ...
    // Internet + AODV
    AodvHelper aodv;
    aodv.Set("ActiveRouteTimeout", StringValue("30s"));  // tempo de expiracao de rota para 30s
    aodv.Set("EnableHello", BooleanValue(false));        // Desativa hello
    aodv.Set("HelloInterval", StringValue("10s"));      // aumenta tempo do hello, caso seja necessario enviar
    ...
    for (uint32_t i = 0; i < nodes.GetN(); ++i) {
        ...
        Ptr<aodv::RoutingProtocol> aodvRouting = DynamicCast<aodv::RoutingProtocol>(rp);
        if (aodvRouting) {
            aodvRouting->TraceConnectWithoutContext("SendRreq", MakeCallback(&ControlPacketTracer));
            aodvRouting->TraceConnectWithoutContext("SendRrep", MakeCallback(&ControlPacketTracer));
            aodvRouting->TraceConnectWithoutContext("SendRerr", MakeCallback(&ControlPacketTracer));
        }
    }

    wifiPhy.EnablePcap("aodv-control", devices);
